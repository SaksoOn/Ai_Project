...
function analyze(history, options = {}) {
    ...
    return verdicts.sort((a, b) => b.flakiness - a.flakiness || b.totalRuns - a.totalRuns || a.id.localeCompare(b.id));
}
function collect(runs) {
    ...
    for (const run of runs) {
        for (const result of run.results) {
            ...
            const observation = { at: run.at, commit: run.commit, result };
            if (bucket)
                bucket.push(observation);
            else
                byTest.set(result.id, [observation]);
        }
    }
    return byTest;
}
/** 같은 커밋에서 2회 이상 실행된 경우만 세고, 그 중 결과가 갈린 것을 찾는다. */
function commitEvidence(observations) {
    ...
    for (const o of observations) {
        ...
        const statuses = byCommit.get(o.commit) ?? new Set();
...
function confidenceOf(comparableCommits, totalRuns) {
    if (comparableCommits >= 3)
        return "high";
    if (comparableCommits >= 1 || totalRuns >= 8)
        return "medium";
    return "low";
}
