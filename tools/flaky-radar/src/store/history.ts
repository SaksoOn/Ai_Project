import { EMPTY_HISTORY, type History, type TestRun } from "../types";
...
export function parseHistory(raw: string | undefined): History {
  ...
  try {
    ...
    if (parsed?.version !== 1 || !Array.isArray(parsed.runs)) return EMPTY_HISTORY;
...
export class HistoryStore {
  ...
  async append(run: TestRun): Promise<History> {
  ...
  async clear(): Promise<void> {
    await this.io.write(serializeHistory(EMPTY_HISTORY));
  }
}

/** 테스트·개발용 인메모리 IO. */
export function memoryIO(initial?: string): HistoryIO & { current(): string | undefined } {
