...
export async function resolveCommit(git: GitReader): Promise<string | undefined> {
  ...
  if (loose && SHA.test(loose)) return loose.toLowerCase();

  // 없으면 packed-refs를 뒤진다. `git gc` 이후에는 이쪽에만 남는다.
  return findInPackedRefs(await git.read("packed-refs"), ref);
}

function findInPackedRefs(contents: string | undefined, ref: string): string | undefined {
  ...
  for (const line of contents.split("\n")) {
    ...
    if (name === ref && sha && SHA.test(sha)) return sha.toLowerCase();
  }
  return undefined;
}
