...
exports.describeRevocation = describeRevocation;
const gumroad_1 = require("./gumroad");
...
class LicenseGate {
    ...
    now;
    fetchImpl;
    constructor(config, storage, deps = {}) {
        this.storage = storage;
        this.productId = config.productId;
        this.trialMs = (config.trialDays ?? 7) * types_1.DAY_MS;
        this.reverifyMs = (config.reverifyAfterDays ?? 7) * types_1.DAY_MS;
        this.graceMs = (config.offlineGraceDays ?? 14) * types_1.DAY_MS;
    ...
    async activate(rawKey) {
        ...
        const result = await (0, gumroad_1.verifyLicense)(this.productId, key, this.fetchImpl);
        if (result.outcome === "unreachable") {
            return {
                ok: false,
                reason: "offline",
                message: `Gumroad에 연결하지 못했습니다 (${result.detail}). ` +
    ...
    // ── 내부 ────────────────────────────────────────────────
    async checkLicensed(state, key, now) {
    ...
    async checkTrial(state, now) {
        ...
        return {
            status: "trial",
            endsAt,
            daysRemaining: Math.max(0, Math.ceil((endsAt - now) / types_1.DAY_MS)),
        };
    }
    /**
...
function describeRevocation(reason) {
    switch (reason) {
        case "invalid-key":
